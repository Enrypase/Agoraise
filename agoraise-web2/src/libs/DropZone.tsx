...
import { convertImg } from "./functions";

export default function DropZone(
  props: ParentProps & {
    previewCallback?: (_data: string[]) => void;
    type: "image" | "any";
  ...
  async function handleOnChange(files: FileList | null) {
    ...
    for (const file of Array.from(files)) {
      if (!acceptTypes()) {
        ...
        if (!acceptTypes()!.full.some((acpt) => acpt === `.${file.type.split("/").at(-1)}`)) {
        ...
        const convertedFiles = await Promise.all(
          acceptTypes()!.preferred.map(async (attempt) => {
            const newImg = await convertImg(file, `${props.type}/${attempt.replaceAll(".", "")}`);
            return newImg;
          }),
        );
    ...
    const inputElement = document.getElementById(`dropZoneInput_${props.name}`) as HTMLInputElement;
