...
function EmailInput(props: LocalInputType) {
  ...
  const valid = createMemo(() => {
    const validation = props.value ? emailSchema.safeParse(props.value) : emailSchema.safeParse(localValue());
    if (localValue() || props.value) {
      setLocalError(validation.error?.errors[0].message || props.error || "");
      props.setError?.(validation.error?.errors[0].message || props.error || "");
    }
    ...
    <div class={commonMainStyle}>
      ...
      >
        <input
          ...
          onInput={(e) => {
            props.setValue ? props.setValue(e.target.value.toLowerCase()) : setLocalValue(e.target.value.toLowerCase());
          }}
          type="email"
          data-type="email"
...
function PasswordInput(props: LocalInputType) {
  ...
  const error = () => props.error || localError();
  const valid = createMemo(() => {
    const validation = props.value ? passwordSchema.safeParse(props.value) : passwordSchema.safeParse(localValue());
    if (localValue() || props.value) {
      setLocalError(validation.error?.errors[0].message || props.error || "");
      props.setError?.(validation.error?.errors[0].message || props.error || "");
    }
        ...
        <For each={props.options}>
          {(opt) => (
            <div class="flex items-center gap-2">
              <button
                type="button"
                onClick={() => (opt.value === state() ? setState("") : setState(opt.value))}
              ...
              >
                <Show when={opt.value === state()}>
                  <PopIn>
                    <svg class="grow" width="100%" height="100%" viewBox="0 0 20 20" fill="none">
                      <path
                        stroke="currentColor"
...
export default function Form(props: {
  ...
  autocomplete?: "off" | "on";
  class?: JSX.HTMLAttributes<HTMLElement>["class"];
  successCallback?: (_inputValues: Record<string, FormDataEntryValue> | FormData) => void;
  asyncSuccessCallback?: (_inputValues: Record<string, FormDataEntryValue> | FormData) => Promise<void>;
  errorCallback?: () => void;
  submitText?: string;
      ...
      if (el.dataset.type === "phone" && el.required) {
        ...
        obj[el.name] = z
          ...
          .refine((file) => file && file.name && file.size, {
            message: `${i}-${el.dataset.validationErrorMsg || "This file is required"}`,
          })
          .refine((file) => !file.name.includes("/"), {
            message: `${i}-${el.dataset.validationErrorMsg || "File name must not include '/'"}`,
          })
          .refine((file) => file && file.size <= MAX_SIZE, {
            message: `${i}-${el.dataset.validationErrorMsg || "The maximum size of the file is 100MB"}`,
          });
        ...
        if (arr) {
          ...
          obj[el.name] = z.string().refine((val) => values.includes(val.toLowerCase()), {
            message: `${i}-${
              el.dataset.validationErrorMsg || `Please insert a valid ${el.title?.toLowerCase() || "value"}`
            }`,
          });
        ...
        obj[el.name] = z.string().email({
          message: `${i}-${el.dataset.validationErrorMsg || `Invalid email provided`}`,
        });
      } else if (el.type === "password" && el.required) {
        obj[el.name] = z
          .string()
          .min(8, {
            message: `${i}-${el.dataset.validationErrorMsg || `The password should be at least 8 chars long`}`,
          })
          ...
          .refine((val) => /[A-Z]/.test(val), {
            message: `${i}-${
              el.dataset.validationErrorMsg || `The passwrord should contain at least a upper case letter`
            }`,
          });
      }
    });
    ...
    if (props.asyncSuccessCallback) {
      try {
        ...
        if (e instanceof AxiosError) {
          ...
          const error = props.errorMessages && props.errorMessages[status][code];
          return setFormError(error || defaultErrorMessages[status][code] || "Error");
        }
        console.error(e);
      }
    }
  ...
  return (
    <form autocomplete={props.autocomplete || "off"} class={props.class} onSubmit={handleSubmit}>
      {resolved}
      <Show when={formError()}>
